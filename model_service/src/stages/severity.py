...
def estimate_severity(context: dict) -> dict:
    ...
    # Generate Grad-CAM / Severity Heatmap Overlay
    try:
        gray = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2GRAY)
        _, rough_leaf_mask = cv2.threshold(gray, 10, 255, cv2.THRESH_BINARY)

        # 1. Start with background (255 = Red in JET colormap)
        activation = np.full(image_bgr.shape[:2], 255, dtype=np.uint8)

        # 2. Healthy leaf area (140 = Yellow/Green in JET colormap)
        activation[rough_leaf_mask > 0] = 140

        # 3. Disease area (0 = Blue in JET colormap)
        if disease_mask is not None and np.count_nonzero(disease_mask) > 0:
        ...
        # 6. Blend with original image (40% leaf, 60% heatmap)
        overlay = cv2.addWeighted(image_bgr, 0.4, heatmap, 0.6, 0)

        # 7. Encode to JPEG and store as base64 string
        success, encoded_buf = cv2.imencode(".jpg", overlay, [int(cv2.IMWRITE_JPEG_QUALITY), 90])
        if success:
            import base64
            context["image"]["processed_image_base64"] = base64.b64encode(encoded_buf.tobytes()).decode("ascii")
